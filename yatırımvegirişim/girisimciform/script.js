...
//tüm dünya şehirlerini içeren bir json dosyasından çekilen 
//"şehirler listesi"nden kullanıcı girdisi aranıyor ve eşleşmeler listeleniyor.
// Ahmet buradaki görüntülenen eşleşmeler çerçevesiz bir alanda görüntüleniyor. düzeltmeni rica ederim.
...
function ekipArkadasiEkle() {
    ...
    ekipDiv.classList.add('row', 'mb-3', 'ekipArkadasi'); // Bootstrap row ve mb-3 sınıflarını ekledik

    // İsim Alanı
    var isimDiv = document.createElement('div');
    isimDiv.classList.add('col'); // Bootstrap kol sınıfını ekledik
    ...
    yeniIsimInput.classList.add('form-control'); // Bootstrap form-control sınıfını ekledik

    isimDiv.appendChild(yeniIsimInput);

    // LinkedIn Alanı
    var linkedInDiv = document.createElement('div');
    linkedInDiv.classList.add('col'); // Bootstrap kol sınıfını ekledik
    ...
    yeniLinkedInInput.classList.add('form-control'); // Bootstrap form-control sınıfını ekledik
    yeniLinkedInInput.addEventListener('blur', linkedInDogrula);

    linkedInDiv.appendChild(yeniLinkedInInput);

    // Departman Alanı
    var departmanDiv = document.createElement('div');
    departmanDiv.classList.add('col'); // Bootstrap kol sınıfını ekledik

    var yeniDepartmanSelect = document.createElement('select');
    yeniDepartmanSelect.setAttribute('name', 'ekipArkadasiDepartman[]');
    yeniDepartmanSelect.classList.add('form-select'); // Bootstrap form-select sınıfını ekledik
    ...
    departmanDiv.appendChild(yeniDepartmanSelect);

    // Pozisyon Alanı
    var pozisyonDiv = document.createElement('div');
    pozisyonDiv.classList.add('col'); // Bootstrap kol sınıfını ekledik

    var yeniPozisyonSelect = document.createElement('select');
    yeniPozisyonSelect.setAttribute('name', 'ekipArkadasiPozisyon[]');
    yeniPozisyonSelect.classList.add('form-select'); // Bootstrap form-select sınıfını ekledik
    ...
    pozisyonDiv.appendChild(yeniPozisyonSelect);

    ekipDiv.appendChild(isimDiv);
    ekipDiv.appendChild(linkedInDiv);
    ekipDiv.appendChild(departmanDiv);
    ekipDiv.appendChild(pozisyonDiv);
...
function sosyalMedyaEkle() {
    ...
    yeniInput.setAttribute('class', 'form-control'); // Bootstrap form-control sınıfını ekleyin
    yeniInput.setAttribute('name', 'sosyalMedya[]');
    yeniInput.setAttribute('placeholder', 'Sosyal Medya Linki');

    // Oluşturulan input elementini ekle
